...
<head>
    <meta charset="utf-8" />
    <title>Demo Whatsapp Numbers</title>
...
<body style="background-color:#F0F0F0;">
    ...
    <div class="modal fade" id="deleteAll" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="delete-whatsapp-numbers-all.php"
                    method="GET">
    ...
    <div class="modal fade" id="downloadExcel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="export-whatsapp-excel.php"
                    method="GET">
    ...
    <div class="mx-12">
        ...
        <h2 class="text-3xl mb-3 font-bold text-red-600">Demo Whatsapp Numbers</h2>

        <?php
        $date = date("Y-m-d", strtotime("yesterday"));
        $sql = "SELECT * FROM demo_whatsapp_contacts WHERE DATE(createdAt) = '$date' and isDeleted = 0 order by id desc";
        ...
        <div>Yesterday Total (App) - <?= $totalApp ?></div>
        <div>Yesterday Total (Web) - <?= $totalWeb ?></div>
        <div>Yesterday Total (Form) - <?= $totalForm ?></div>

        <?php

        $sql_total = "select COUNT(id) as total_users from demo_whatsapp_contacts where isDeleted = 0";
        ...
        $sql_total = "SELECT COUNT(id) as total_users FROM demo_whatsapp_contacts WHERE " . $whereClause . " order by id desc ";

        if (isset($_GET['sort']) && $_GET['sort'] == "app")
            $sql_total = "SELECT COUNT(id) as total_users FROM demo_whatsapp_contacts WHERE platform='app' and " . $whereClause . " order by id desc ";

        if (isset($_GET['sort']) && $_GET['sort'] == "web")
            $sql_total = "SELECT COUNT(id) as total_users FROM demo_whatsapp_contacts WHERE platform='web' and " . $whereClause . " order by id desc ";

        if (isset($_GET['sort']) && $_GET['sort'] == "form")
            $sql_total = "SELECT COUNT(id) as total_users FROM demo_whatsapp_contacts WHERE platform='form' and " . $whereClause . " order by id desc ";
                ...
                <div class="dropdown show">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        <a class="dropdown-item"
                            href="?<?= http_build_query(array_merge($_GET, array('sort' => 'web'))) ?>">Sort by Web</a>
                        <a class="dropdown-item"
                            href="?<?= http_build_query(array_merge($_GET, array('sort' => 'app'))) ?>">Sort by App</a>
                        <a class="dropdown-item"
        ...
        $sql = "SELECT * FROM demo_whatsapp_contacts WHERE " . $whereClause . " order by id desc $limitQuery";

        if (isset($_GET['sort']) && $_GET['sort'] == "app")
            $sql = "SELECT * FROM demo_whatsapp_contacts WHERE platform='app' and " . $whereClause . " order by id desc $limitQuery";

        if (isset($_GET['sort']) && $_GET['sort'] == "web")
            $sql = "SELECT * FROM demo_whatsapp_contacts WHERE platform='web' and " . $whereClause . " order by id desc $limitQuery";

        if (isset($_GET['sort']) && $_GET['sort'] == "form")
            $sql = "SELECT * FROM demo_whatsapp_contacts WHERE platform='form' and " . $whereClause . " order by id desc $limitQuery";
        ...
        <table class="w-full table-auto bg-white border-collapse" id="myTable">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-100 border-b border-dashed">
                    ...
                    <td class="px-2 py-4 text-sm text-center"> <?= $row["mobile"]; ?>
                    <td class="px-2 py-4 text-sm text-center"> <?= $row["platform"]; ?>
                    </td>
                    <td class="px-2 py-4 text-sm text-center">
                        <?= date_format(date_create($row["createdAt"]), "d-M-Y h:i:sa"); ?></td>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4"
                                        action="delete-whatsapp-numbers-all.php" method="GET">
