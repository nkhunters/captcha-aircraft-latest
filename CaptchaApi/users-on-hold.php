...
<body style="background-color:#F0F0F0;">
    ...
    <div class="mx-12">
        <h2 class="text-3xl mb-3 font-bold text-red-600">Users on hold</h2>
        ...
        <?php
            ...
            $sql = "SELECT user_id, COUNT(user_id) as total, date_time, platform, on_hold FROM users where on_hold = 1 AND user_id like '$page'";
            if (isset($_GET['web'])) {
                $sql = "SELECT user_id, COUNT(user_id) as total, date_time, platform, on_hold FROM users where on_hold = 1 and platform = 'web' AND user_id like '$page'";
            }

            if (isset($_GET['app'])) {
                $sql = "SELECT user_id, COUNT(user_id) as total, date_time, platform, on_hold FROM users where on_hold = 1 and platform = 'app' AND user_id like '$page'";
            }

            if (isset($_GET['both'])) {
                $sql = "SELECT user_id, COUNT(user_id) as total, date_time, platform, on_hold FROM users where on_hold = 1 and platform = 'both' AND user_id like '$page'";
            }

            if (isset($_GET['date'])) {
                $sql = "SELECT user_id, COUNT(user_id) as total, date_time, platform, on_hold, STR_TO_DATE(date_time, '%d-%b-%Y %h:%i:%s%p') as created_on FROM users where on_hold = 1 AND user_id like '$page'  order by created_on desc";
            }
        ...
        <table class="w-full table-auto bg-white border-collapse" id="myTable">
            ...
            <tbody>
                <?php
                    ...
                    $j = 1;
                    $sql2 = "SELECT user_id, date_time, platform, on_hold, STR_TO_DATE(date_time, '%d-%b-%Y %h:%i:%s%p') as created_on FROM users where on_hold = 1 AND user_id like '$page' order by created_on desc $limitQuery";
                    if (isset($_GET['web'])) {
                        $sql2 = "SELECT user_id, date_time, platform, on_hold FROM users where  platform = 'web' AND on_hold = 1 AND user_id like '$page' $limitQuery";
                    }

                    if (isset($_GET['app'])) {
                        $sql2 = "SELECT user_id, date_time, platform, on_hold FROM users where  platform = 'app' AND on_hold = 1 AND user_id like '$page' $limitQuery";
                    }

                    if (isset($_GET['both'])) {
                        $sql2 = "SELECT user_id, date_time, platform, on_hold FROM users where  platform = 'both' AND on_hold = 1 AND user_id like '$page' $limitQuery";
                    }
